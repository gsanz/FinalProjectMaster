var NuevaRuta = Backbone.View.extend({ 
	...
	pararRuta: function() {
		...
		clearInterval(this.timerGps);  	
  		// guardar ruta
		this.collection.add(this.model); // limpiar modelo
		this.model = null;	
		// pintar vista
		this.render(); 
	},
	...
	render: function() { 
		if (this.grabando) {
			...
			minutos = minutos%60;
			this.$('#lblReloj').text("" + (horas<10? "0": "") + horas + ":" +(minutos<10? "0": "") + minutos + ":" + (segundos < 10? "0": "") + segundos); 
